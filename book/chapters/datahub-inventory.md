...
**deterministically derived layers**, to **model outputs** — with sources,
...
**Native support** records the true footprint a value
...
| Layer / product | Serves | Source · archive (API / website) | Native support | Posting resolution · CRS | Temporal resolution | Units | Key limitations |
|---|---|---|---|---|---|---|---|
...
| **Soil texture & properties** → `sand__total`, `silt__total`, `clay__total`, `soil__ph` (1:1 H₂O), `soil__organic_carbon`, `dry__bulk_density`, `cation__exchange_capacity`, `water_content__third_bar` (field capacity), `water_content__fifteen_bar` (wilting point), `coarse_fragment__volume`, `soil__depth_to_restriction` | ⛰️ 🔥 🏚️ | USDA SOLUS100 (100 m); public GCS `solus100pub`; STAC [`solus-stac`](https://github.com/gaia-hazlab/solus-stac) | ML estimate 📈; effective support coarser than grid (SSURGO-scale training) | 100 m · EPSG:5070; depths 0,5,15,30,60,100,150 cm | Static (ML estimate) | %, pH, g cm⁻³, cmol(+)/kg, m³ m⁻³, cm | ML-predicted with **uncertainty bands (l/h)** per property; CONUS-only; vocabulary differs from POLARIS |
| **Soil hydraulic & texture priors** (alt.) → texture `sand`, `silt`, `clay`; bulk density `bd`; saturated / residual water content `theta_s` / `theta_r`; sat. hydraulic conductivity `ksat`; van Genuchten retention `alpha` / `n`; Brooks–Corey `lambda` / `hb`; organic matter `om`; `ph` | ⛰️ 🏚️ | POLARIS 30 m ([hydrology.cee.duke.edu/POLARIS](http://hydrology.cee.duke.edu/POLARIS/)); used by [`landslide-digital-twin`](https://github.com/gaia-hazlab/landslide-digital-twin) | Downscaled from SSURGO 📈 (coarser than grid) | 30 m; same depth scheme; p5/p50/p95 | Static (statistical) | %; g cm⁻³; m³ m⁻³; `ksat` cm hr⁻¹ (log₁₀); `alpha`, `hb` kPa; `lambda`, `n` – | Downscaled SSURGO; **different vocabulary, depths, stats & units from SOLUS** — conversion table required; several layers stored as **log₁₀** (`ksat`, `alpha`, `hb`, `om`); φ / cohesion bounds & field capacity / wilting point are **derived** from this stack (§2), not native POLARIS layers |
| **Shear-wave velocity** → $V_{s30}$, $V_s(z)$ | 🏚️ ⛰️ | parametric CONUS $V_s$ [@sanger2025vs]; USGS National Crustal Model; slope/geology proxy $V_{s30}$ | Proxy ~250–1000 m 📈; parametric at site | Parametric / gridded proxy | Static (→ dynamic via seismic) | m s⁻¹ | Proxy $V_{s30}$ has large scatter; **rigidity is high-influence** for liquefaction |
| **Surficial geology / soil type** → `geologic__unit` (mapped formation), `deposit__age`, `lithology__class`, `depositional__genesis` (fluvial / lacustrine / glacial / artificial fill), `liquefaction__susceptibility_class` | 🏚️ ⛰️ | state geologic surveys; USGS | Map scale 1:24k–1:100k | Vector (1:24k–1:100k) | Static | categorical | Map-scale generalization; susceptibility class boundaries uncertain |
| **Landcover** → `vegetation__plant_functional_type` | ⛰️ 🔥 | USGS/MRLC NLCD ([mrlc.gov](https://www.mrlc.gov/)) | 30 m | 30 m | ~2–3 yr epochs | categorical | Class generalization; epoch lag; needs class→PFT lookup |
